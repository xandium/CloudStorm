...
class DiscordConnector extends EventEmitter {
  ...
  messageAction(message) {
    ...
    if (message.s) {
      if (message.s > this.seq + 1) {
        this.client.emit(
          "debug",
          `Shard ${this.id}, invalid sequence: current: ${this.seq} message: ${
            message.s
          }`
        );
        this.seq = message.s;
    ...
    let data = {
      ...
      d: {
        ...
        presence: this.options.initialPresence
          ? this._checkPresenceData(this.options.initialPresence)
          : null
      }
    };
    this.forceIdentify = false;
  ...
  statusUpdate(data = {}) {
    return this.betterWs.sendMessage({
      op: OP.STATUS_UPDATE,
      d: this._checkPresenceData(data)
    });
  }
  ...
  voiceStateUpdate(data) {
    ...
    return this.betterWs.sendMessage({
      ...
      d: this._checkVoiceStateUpdateData(data)
    });
  }
  ...
  requestGuildMembers(data = {}) {
    return this.betterWs.sendMessage({
      ...
      d: this._checkRequestGuildMembersData(data)
    });
  }
